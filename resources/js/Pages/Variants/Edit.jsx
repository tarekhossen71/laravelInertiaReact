...
export default function VariantEdit({ pageTitle, products, variant }) {
    const { errors, flash } = usePage().props;

    const [product_id, setProduct_id] = useState(variant.product_id || '');
    const [size, setSize] = useState(variant.size?.map(s => ({ value: s, label: s })) || []);
    const [color, setColor] = useState(variant.color?.map(c => ({ value: c, label: c })) || []);
    const [sleeve_type, setSleeve_type] = useState(variant.sleeve_type?.map(s => ({ value: s, label: s })) || []);
    const [collar_type, setCollar_type] = useState(variant.collar_type?.map(c => ({ value: c, label: c })) || []);
    const [price, setPrice] = useState(variant.price || '');
    const [stock_qty, setStock_qty] = useState(variant.stock_qty || '');
    ...
    const handleSubmit = (e) => {
        ...
        router.post(route('app.variant.update', variant.id), {
            product_id,
    ...
    return (
        <AdminLayout>
            ...
            <Breadcrumb pageTitle={pageTitle} breadcrumb={[{ name: pageTitle, url: route('app.variant.index') }, { name: 'Edit' }]} />
                            ...
                            <form onSubmit={handleSubmit} encType="multipart/form-data">
                                <div className="row">
                                    ...
                                    <div className="col-md-4">
                                        ...
                                        <Select isMulti options={sizeOptions} value={size} onChange={setSize} />
                                        {errors.size && <div className="text-danger">{errors.size}</div>}
                                    </div>

                                    <div className="col-md-4">
                                        <label className="form-label">Color</label>
                                        <Select isMulti options={colorOptions} value={color} onChange={setColor} />
                                        {errors.color && <div className="text-danger">{errors.color}</div>}
                                    </div>

                                    <div className="col-md-4">
                                        <label className="form-label">Sleeve Type</label>
                                        <Select isMulti options={sleeveOptions} value={sleeve_type} onChange={setSleeve_type} />
                                        {errors.sleeve_type && <div className="text-danger">{errors.sleeve_type}</div>}
                                    </div>

                                    <div className="col-md-4">
                                        <label className="form-label">Collar Type</label>
                                        <Select isMulti options={collarOptions} value={collar_type} onChange={setCollar_type} />
